...
export function runHealthcheck(con, id) {
    return dockerJson("/containers/" + encodeURIComponent(id) + "/exec", "POST", {}, JSON.stringify({
        Cmd: ["sh", "-c", "exit 0"], // Simple healthcheck command
        AttachStdout: false,
        AttachStderr: false
    }));
}
