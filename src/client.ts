...
export const streamContainerStats = (con: Connection, callback: MonitorCallback) =>
    con.monitor(VERSION + "/containers/stats", callback);
...
export const renameContainer = (con: Connection, id: string, config: JsonObject) => dockerCall(con, "/containers/" + id + "/rename", "POST", config);
...
export const runHealthcheck = (con: Connection, id: string) => dockerCall(con, "/containers/" + id + "/healthcheck", "GET", {});

export const postPod = (con: Connection, action: string, id: string, args: JsonObject) => dockerCall(con, "/pods/" + id + "/" + action, "POST", args);

export const delPod = (con: Connection, id: string, force: boolean) => dockerCall(con, "/pods/" + id, "DELETE", { force });

export const createPod = (con: Connection, config: JsonObject) => dockerCall(con, "/pods/create", "POST", {}, JSON.stringify(config));
...
export const untagImage = (con: Connection, id: string, repo: string, tag: string) => dockerCall(con, "/images/" + id + "/untag", "POST", { repo, tag });

export function pullImage(con: Connection, reference: string) {
    return new Promise<void>((resolve, reject) => {
        dockerCall(con, "/images/pull", "POST", { reference })
                .then(r => {
...
export const pruneUnusedImages = (con: Connection) => dockerJson(con, "/images/prune?all=true", "POST", {});

export const imageHistory = (con: Connection, id: string) => dockerJson(con, `/images/${id}/history`, "GET", {});

export const imageExists = (con: Connection, id: string) => dockerCall(con, "/images/" + id + "/exists", "GET", {});

export const containerExists = (con: Connection, id: string) => dockerCall(con, "/containers/" + id + "/exists", "GET", {});
